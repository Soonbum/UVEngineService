...
namespace UVEngine.Controllers.CyUSBSerial;

public class CyUSBSerial
{
    // This section contains details of the all the constants that are part of Cypress USB Serial driver library.
    public const int CY_STRING_DESCRIPTOR_SIZE = 256;   // String descriptor size (DEFINE 상수)
    public const int CY_MAX_DEVICE_INTERFACE = 5;       // Maximum number of interfaces (DEFINE 상수)

    // Major version number for library
    public const int CY_US_VERSION_MAJOR = 2;

    // Minor version number for library
    public const int CY_US_VERSION_MINOR = 0;

    // Patch version number for library
    public const int CY_US_VERSION_PATCH = 3;

    // Version number for the device
    public const int CY_US_VERSION = ((CY_US_VERSION_MAJOR) | (CY_US_VERSION_MINOR << 8) | (CY_US_VERSION_PATCH << 16));

    // Library build number
    public const int CY_US_VERSION_BUILD = 91;
    ...
    */
    ...
    {
        public UInt16 vid;  // Vendor ID
        public UInt16 pid;  // Product ID
    }
    ...
    */
    ...
    {
        public byte majorVersion;  // The major version of the library (byte는 UInt8)
        public byte minorVersion;  // The minor version of the library (byte는 UInt8)
        public UInt16 patch;       // The patch number of the library
        public byte buildNumber;   // The build number of the library (byte는 UInt8)
    }
    ...
    */
    [StructLayout(LayoutKind.Sequential)]
    public struct CY_FIRMWARE_VERSION
    {
        public byte majorVersion;  // Major version of the Firmware (byte는 UInt8)
        public byte minorVersion;  // Minor version of the Firmware (byte는 UInt8)
        public UInt16 patchNumber; // Patch Number of the Firmware
        public UInt32 buildNumber; // Build Number of the Firmware
    }
    ...
    * CyGetDeviceInfoVidPid
    */
    public enum CY_DEVICE_CLASS : int   // 4바이트 enum
    {
        CY_CLASS_DISABLED = 0x00,  // None or the interface is disabled
    ...
    * CyGetDeviceInfoVidPid
    */
    public enum CY_DEVICE_TYPE : int    // 4바이트 enum
    {
    ...
    CyGetDeviceInfoVidPid
    */
    public enum CY_DEVICE_SERIAL_BLOCK : int    // 4바이트 enum
    {
    ...
    */
    ...
    {
        public CY_VID_PID vidPid;                               // VID and PID
        public byte numInterfaces;                              // Number of interfaces supported (UCHAR)

        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = CY_STRING_DESCRIPTOR_SIZE)]
        public string manufacturerName;                         // Manufacturer name (UCHAR[CY_STRING_DESCRIPTOR_SIZE] -> string, 문자열의 경우 ByValTStr)

        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = CY_STRING_DESCRIPTOR_SIZE)]
        public string productName;                              // Product name (UCHAR[CY_STRING_DESCRIPTOR_SIZE] -> string, 문자열의 경우 ByValTStr)

        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = CY_STRING_DESCRIPTOR_SIZE)]
        public string serialNum;                                // Serial number (UCHAR[CY_STRING_DESCRIPTOR_SIZE] -> string, 문자열의 경우 ByValTStr)

        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = CY_STRING_DESCRIPTOR_SIZE)]
        public string deviceFriendlyName;                       // Device friendly name : Windows only (UCHAR[CY_STRING_DESCRIPTOR_SIZE] -> string, 문자열의 경우 ByValTStr)

        [MarshalAs(UnmanagedType.ByValArray, SizeConst = CY_MAX_DEVICE_INTERFACE)]
        public CY_DEVICE_TYPE[] deviceType;                     // Type of the device each interface has(Valid only for USB Serial Device) and interface in vendor class (CY_DEVICE_TYPE[CY_MAX_DEVICE_INTERFACE], 일반 배열의 경우 ByValArray)

        [MarshalAs(UnmanagedType.ByValArray, SizeConst = CY_MAX_DEVICE_INTERFACE)]
        public CY_DEVICE_CLASS[] deviceClass;                   // Interface class of each interface (CY_DEVICE_CLASS[CY_MAX_DEVICE_INTERFACE], 일반 배열의 경우 ByValArray)

        public CY_DEVICE_SERIAL_BLOCK deviceBlock;              // On Windows, each USB Serial device interface is associated with a separate driver instance.
                                                                // This variable represents the present driver interface instance that is associated with a serial block.
    }
    ...
    */
    ...
    {
        public IntPtr buffer;           // Pointer to the buffer from where the data is read/written (UCHAR*)
    ...
    */
    ...
    {
        public UInt32 frequency;        // I2C clock frequency 1KHz to 400KHz
        public byte slaveAddress;       // Slave address of the I2C module, when it is configured as slave (UINT8)
        public bool isMaster;           // true- Master , false- slave
        public bool isClockStretch;     // true- Stretch clock in case of no data availability (Valid only for slave mode)
    ...
    */
    ...
    {
        public byte slaveAddress;      // Slave address the master will communicate with (UCHAR)
        public bool isStopBit;         // Set when stop bit is used
        public bool isNakBit;          // Set when I2C master wants to NAK the slave after read Applicable only when doing I2C read
    }
    ...
    * CySetSpiConfig
    */
    public enum CY_SPI_PROTOCOL : int       // 4바이트 enum
    {
    ...
    */
    ...
    public struct CY_SPI_CONFIG
    {
        public UInt32 frequency;               // SPI clock frequency.
                                               // IMPORTANT: The frequency range supported by SPI module is 1000(1KHz) to 3000000(3MHz)

        public byte dataWidth;                 // Data width in bits. The valid values are from 4 to 16. (UCHAR)

        public CY_SPI_PROTOCOL protocol;       // SPI Protocols to be used as defined in CY_SPI_PROTOCOL

        public bool isMsbFirst;                // false -> least significant bit is sent out first true -> most significant bit is sent out first

        public bool isMaster;                  // false --> Slave mode selected: true --> Master mode selected

        public bool isContinuousMode;          // true - Slave select line is not asserted i.e  de-asserted for every word.
    ...
    * CyGetUartConfig
    */
    public enum CY_UART_BAUD_RATE : int     // 4바이트 enum
    {
    ...
    * CyGetUartConfig
    */
    public enum CY_UART_PARITY_MODE : int   // 4바이트 enum
    {
    ...
    * CyGetUartConfig
    */
    public enum CY_UART_STOP_BIT : int      // 4바이트 enum
    {
    ...
    */
    public enum CY_FLOW_CONTROL_MODES : int // 4바이트 enum
    {
    ...
    */
    [StructLayout(LayoutKind.Sequential)]
    public struct CY_UART_CONFIG
    {
        public CY_UART_BAUD_RATE baudRate;         // Baud rate as defined in CY_UART_BAUD_RATE
        public byte dataWidth;                     // Data width: valid values 7 or 8 (UINT8)
        public CY_UART_STOP_BIT stopBits;          // Number of stop bits to be used 1 or 2
        public CY_UART_PARITY_MODE parityMode;     // UART parity mode as defined in CY_UART_PARITY_MODE
        public bool isDropOnRxErrors;              // Whether to ignore framing as well as parity errors and receive data
    }
    ...
    */
    public enum CY_CALLBACK_EVENTS : int    // 4바이트 enum
    {
    ...
    */
    /*
       ...
       * CyLibraryExit
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyLibraryInit")]
    public static extern CY_RETURN_STATUS CyLibraryInit();
    ...
    /*
       ...
       * CyLibraryInit
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyLibraryExit")]
    public static extern CY_RETURN_STATUS CyLibraryExit();
    ...
    /*
       ...
       * CyClose
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyGetListofDevices")]
    public static extern CY_RETURN_STATUS CyGetListofDevices(
        ref byte numDevices     // Number of Devices connected (UINT8* numDevices)
    );
    ...
    /*
       ...
       * CyClose
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyGetDeviceInfo")]
    public static extern CY_RETURN_STATUS CyGetDeviceInfo(
        byte deviceNumber,              // Device number of the device of interest (UINT8 deviceNumber)
        ref CY_DEVICE_INFO deviceInfo   // Info of device returned (CY_DEVICE_INFO* deviceInfo)
    );
    ...
    /*
       ...
       * CyClose
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyGetDeviceInfoVidPid")]
    public static extern CY_RETURN_STATUS CyGetDeviceInfoVidPid(
        CY_VID_PID vidPid,                              // VID and PID of device of interest
        [In, Out] byte[] deviSerialBlock_SCB0ceIdList,  // Array of device ID's returned (UINT8* deviSerialBlock_SCB0ceIdList)
        [In, Out] CY_DEVICE_INFO[] deviceInfoList,      // Array of pointers to device info list (CY_DEVICE_INFO* deviceInfoList)
        ref byte deviceCount,                           // Count of devices with specified VID PID (UINT8* deviceCount)
        byte infoListLength                             // Total length of the deviceInfoList allocated (Size of deviceInfoList array) (UINT8 infoListLength)
    );
    ...
    /*
       ...
       * CyClose
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyOpen")]
    public static extern CY_RETURN_STATUS CyOpen(
        byte deviceNumber,      // Device number of device that needs to be opened (UINT8 deviceNumber)
        byte interfaceNum,      // Interface Number (UINT8 interfaceNum)
        ref IntPtr handle       // Handle returned by the API (CY_HANDLE* or void**)
    );
    ...
    /*
       ...
       * CyOpen
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyClose")]
    public static extern CY_RETURN_STATUS CyClose(
        IntPtr handle       // Handle of the device that needs to be closed (CY_HANDLE or void*)
    );
    ...
    /*
       ...
       * CyResetDevice
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyCyclePort")]
    public static extern CY_RETURN_STATUS CyCyclePort(
        IntPtr handle       // Valid device handle (CY_HANDLE or void*)
    );
    ...
    */
    ...
    /*
       ...
       * CyGetGpioValue
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CySetGpioValue")]
    public static extern CY_RETURN_STATUS CySetGpioValue(
        IntPtr handle,      // Valid device handle (CY_HANDLE or void*)
        byte gpioNumber,    // GPIO number (UINT8 gpioNumber)
        byte value          // Value that needs to be set (UINT8 value)
    );
    ...
    /*
       ...
       * CySetGpioValue
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyGetGpioValue")]
    public static extern CY_RETURN_STATUS CyGetGpioValue(
        IntPtr handle,      // Valid device handle (CY_HANDLE or void*)
        byte gpioNumber,    // GPIO number (UINT8 gpioNumber)
        out byte value      // Current state of the GPIO (UINT8* value)
    );
    ...
    */
    ...
    /*
       ...
       * CySetEventNotification
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyAbortEventNotification")]
    public static extern CY_RETURN_STATUS CyAbortEventNotification(
        IntPtr handle       // Valid device handle (CY_HANDLE or void*)
    );
    ...
    /*
       ...
       * CyGetFirmwareVersion 
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyGetLibraryVersion")]
    public static extern CY_RETURN_STATUS CyGetLibraryVersion(
        IntPtr handle,                  // Valid device handle (CY_HANDLE or void*)
        ref CY_LIBRARY_VERSION version  // Library version of the current library (PCY_LIBRARY_VERSION version)
    );
    ...
    /*
       ...
       * CyGetLibraryVersion
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyGetFirmwareVersion")]
    public static extern CY_RETURN_STATUS CyGetFirmwareVersion(
        IntPtr handle,                              // Valid device handle (CY_HANDLE or void*)
        ref CY_FIRMWARE_VERSION firmwareVersion     // Firmware version (PCY_FIRMWARE_VERSION firmwareVersion)
    );
    ...
    /*
       ...
       * CyCyclePort
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyResetDevice")]
    public static extern CY_RETURN_STATUS CyResetDevice(
        IntPtr handle       // Valid device handle (CY_HANDLE or void*)
    );
    ...
    /*
       ...
       * CyReadUserFlash
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyProgUserFlash")]
    public static extern CY_RETURN_STATUS CyProgUserFlash(
        IntPtr handle,                  // Valid device handle (CY_HANDLE or void*)
        ref CY_DATA_BUFFER progBuffer,  // Data buffer containing buffer address, length to write (CY_DATA_BUFFER* progBuffer)
    ...
    /*
       ...
       * CyProgUserFlash
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyReadUserFlash")]
    public static extern CY_RETURN_STATUS CyReadUserFlash(
        IntPtr handle,                  // Valid device handle (CY_HANDLE or void*)
        ref CY_DATA_BUFFER readBuffer,  // Data buffer containing buffer address, length to read (CY_DATA_BUFFER* readBuffer)
    ...
    /*
       ...
       * CY_ERROR_REQUEST_FAILED when request is failed by USB Serial device.
     */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyGetSignature")]
    public static extern CY_RETURN_STATUS CyGetSignature(
        IntPtr handle,          // Valid device handle (CY_HANDLE or void*)
        out byte pSignature     // Signature returned (UCHAR *pSignature)
    );
    ...
    */
    ...
    /*
       ...
       * CySetUartConfig 
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyGetUartConfig")]
    public static extern CY_RETURN_STATUS CyGetUartConfig(
        IntPtr handle,                  // Valid device handle (CY_HANDLE or void*)
        ref CY_UART_CONFIG uartConfig   // UART configuration value read back (CY_UART_CONFIG* uartConfig)
    );
    ...
    /*
       ...
       * CyGetUartConfig 
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CySetUartConfig")]
    public static extern CY_RETURN_STATUS CySetUartConfig(
        IntPtr handle,                  // Valid device handle (CY_HANDLE or void*)
        ref CY_UART_CONFIG uartConfig   // UART configuration value (CY_UART_CONFIG* uartConfig)
    );
    ...
    /*
       ...
       * CyUartWrite
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyUartRead")]
    public static extern CY_RETURN_STATUS CyUartRead(
        IntPtr handle,                  // Valid device handle (CY_HANDLE or void*)
        ref CY_DATA_BUFFER readBuffer,  // Read buffer details (CY_DATA_BUFFER* readBuffer)
    ...
    /*
       ...
       * CyUartRead
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyUartWrite")]
    public static extern CY_RETURN_STATUS CyUartWrite(
        IntPtr handle,                      // Valid device handle (CY_HANDLE or void*)
        ref CY_DATA_BUFFER writeBuffer,     // Write buffer details (CY_DATA_BUFFER* writeBuffer)
        UInt32 timeout                      // API timeout value
    );
    ...
    /*
       ...
       * CyUartGetHwFlowControl
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyUartSetHwFlowControl")]
    public static extern CY_RETURN_STATUS CyUartSetHwFlowControl(
        IntPtr handle,                  // Valid device handle (CY_HANDLE or void*)
    ...
    /*
       ...
       * CyUartSetHwFlowControl
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyUartGetHwFlowControl")]
    public static extern CY_RETURN_STATUS CyUartGetHwFlowControl(
        IntPtr handle,                      // Valid device handle (CY_HANDLE or void*)
        ref CY_FLOW_CONTROL_MODES mode      // Flow control mode (CY_FLOW_CONTROL_MODES* mode)
    );
    ...
    /*
       ...
       * CyUartClearDtr
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyUartSetRts")]
    public static extern CY_RETURN_STATUS CyUartSetRts(
        IntPtr handle       // Valid device handle (CY_HANDLE or void*)
    );
    ...
    /*
       ...
       * CyUartClearDtr
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyUartClearRts")]
    public static extern CY_RETURN_STATUS CyUartClearRts(
        IntPtr handle       // Valid device handle (CY_HANDLE or void*)
    );
    ...
    /*
       ...
       * CyUartClearDtr
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyUartSetDtr")]
    public static extern CY_RETURN_STATUS CyUartSetDtr(
        IntPtr handle       // Valid device handle (CY_HANDLE or void*)
    );
    ...
    /*
       ...
       * CyUartClearRts
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyUartClearDtr")]
    public static extern CY_RETURN_STATUS CyUartClearDtr(
        IntPtr handle       // Valid device handle (CY_HANDLE or void*)
    );
    ...
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyUartSetBreak")]
    public static extern CY_RETURN_STATUS CyUartSetBreak(
        IntPtr handle,      // Valid device handle (CY_HANDLE or void*)
    ...
    */
    ...
    /*
       ...
       * CySetI2cConfig 
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyGetI2cConfig")]
    public static extern CY_RETURN_STATUS CyGetI2cConfig(
        IntPtr handle,                  // Valid device handle (CY_HANDLE or void*)
        ref CY_I2C_CONFIG i2cConfig     // I2C configuration value read back (CY_I2C_CONFIG* i2cConfig)
    );
    ...
    /*
       ...
       * CySetI2cConfig 
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CySetI2cConfig")]
    public static extern CY_RETURN_STATUS CySetI2cConfig(
        IntPtr handle,                  // Valid device handle (CY_HANDLE or void*)
        ref CY_I2C_CONFIG i2cConfig     // I2C configuration value (CY_I2C_CONFIG* i2cConfig)
    );
    ...
    /*
       ...
       * CyI2cCWrite
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyI2cRead")]
    public static extern CY_RETURN_STATUS CyI2cRead(
        IntPtr handle,                      // Valid device handle (CY_HANDLE or void*)
        ref CY_I2C_DATA_CONFIG dataConfig,  // I2C data config (CY_I2C_DATA_CONFIG* dataConfig)
        ref CY_DATA_BUFFER readBuffer,      // Read buffer details (CY_DATA_BUFFER* readBuffer)
        UInt32 timeout                      // API timeout value
    );
    ...
    /*
       ...
       * CyI2cRead
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyI2cWrite")]
    public static extern CY_RETURN_STATUS CyI2cWrite(
        IntPtr handle,                          // Valid device handle (CY_HANDLE or void*)
        ref CY_I2C_DATA_CONFIG dataConfig,      // I2C Slave address (CY_I2C_DATA_CONFIG* dataConfig)
        ref CY_DATA_BUFFER writeBuffer,         // Write buffer details (CY_DATA_BUFFER* writeBuffer)
        UInt32 timeout                          // API timeout value
    );
    ...
    /*
       ...
       * CyI2CWrite
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyI2cReset")]
    public static extern CY_RETURN_STATUS CyI2cReset(
        IntPtr handle,      // Valid device handle (CY_HANDLE or void*)
    ...
    */
    ...
    /*
       ...
       * CySetSpiConfig 
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyGetSpiConfig")]
    public static extern CY_RETURN_STATUS CyGetSpiConfig(
        IntPtr handle,                  // Valid device handle (CY_HANDLE or void*)
        ref CY_SPI_CONFIG spiConfig     // SPI configuration structure value read back (CY_SPI_CONFIG* spiConfig)
    );
    ...
    /*
       ...
       * CyGetSpiConfig
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CySetSpiConfig")]
    public static extern CY_RETURN_STATUS CySetSpiConfig(
        IntPtr handle,                      // Valid device handle (CY_HANDLE or void*)
        ref CY_SPI_CONFIG spiConfig         // SPI configuration structure value (CY_SPI_CONFIG* spiConfig)
    );
    ...
    /*
       ...
       * CySetSpiConfig
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CySpiReadWrite")]
    public static extern CY_RETURN_STATUS CySpiReadWrite(
        IntPtr handle,                      // Valid device handle (CY_HANDLE or void*)
        ref CY_DATA_BUFFER readBuffer,      // Read data buffer (CY_DATA_BUFFER* readBuffer)
        ref CY_DATA_BUFFER writeBuffer,     // Write data buffer (CY_DATA_BUFFER* writeBuffer)
    ...
    */
    ...
    /*
       ...
       * CyJtagDisable
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyJtagEnable")]
    public static extern CY_RETURN_STATUS CyJtagEnable(
        IntPtr handle         // Valid device handle (CY_HANDLE or void*)
    );
    ...
    /*
       ...
       * CyJtagEnable
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyJtagDisable")]
    public static extern CY_RETURN_STATUS CyJtagDisable(
        IntPtr handle         // Valid device handle (CY_HANDLE or void*)
    );
    ...
    /*
       ...
       * CyJtagEnable
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyJtagWrite")]
    public static extern CY_RETURN_STATUS CyJtagWrite(
        IntPtr handle,                      // Valid device handle (CY_HANDLE or void*)
        ref CY_DATA_BUFFER writeBuffer,     // Write buffer details (CY_DATA_BUFFER *writeBuffer)
        UInt32 timeout                      // API timeout value
    );
    ...
    /*
       ...
       * CyJtagEnable
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyJtagRead")]
    public static extern CY_RETURN_STATUS CyJtagRead(
        IntPtr handle,                      // Valid device handle (CY_HANDLE or void*)
        ref CY_DATA_BUFFER readBuffer,      // Read buffer parameters (CY_DATA_BUFFER* readBuffer)
        UInt32 timeout                      // API timeout value
    );
    ...
    */
    ...
    /*
       ...
       * CyPhdcGetStatus
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyPhdcClrFeature")]
    public static extern CY_RETURN_STATUS CyPhdcClrFeature(
        IntPtr handle       // Valid device handle (CY_HANDLE or void*)
    );
    ...
    /*
       ...
       * CyPhdcGetStatus
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyPhdcSetFeature")]
    public static extern CY_RETURN_STATUS CyPhdcSetFeature(
        IntPtr handle       // Valid device handle (CY_HANDLE or void*)
    );
    ...
    /*
       ...
       * CyPhdcSetFeature
    */
    [System.Runtime.InteropServices.DllImportAttribute("cyusbserial.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "CyPhdcGetStatus")]
    public static extern CY_RETURN_STATUS CyPhdcGetStatus(
        IntPtr handle,          // Valid device handle (CY_HANDLE or void*)
        ref UInt16 dataStatus   // Data pending status bit map (UINT16*)
    );
}
